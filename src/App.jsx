...
import { Document, Packer, Paragraph, Table, TableRow, TableCell, TextRun, WidthType, AlignmentType, PageOrientation } from "docx";
...
const ROLES_NAVALES = [
  ...
  // Título "Marinero de Primera de Máquinas" (PNA: "Auxiliar de Máquinas"),
  // en la jerga "Engrasador"; "1er Cabo" es una categoría salarial de ese
  // mismo puesto (antigüedad / único en el puesto a bordo), no un rol
  // distinto — por eso es un solo Rol de embarque.
  { nombre:"Auxiliar de Máquinas / Engrasador / 1er Cabo", jerarquia:"marineria" },
...
const ORDEN_ROL_EMBARQUE = [
  ...
  "Auxiliar de Máquinas / Engrasador / 1er Cabo",
  "Cocinero",
  "Contramaestre",
  "Marinero",
...
const DOTACION_MINIMA = {
  "Atlantic Dama": [
    ...
    { label: "Auxiliar de Máquinas", puestos: ["Auxiliar de Máquinas / Engrasador / 1er Cabo"], cantidad: 1 },
  ],
  "Golondrina de Mar": [
...
// Genera la Planilla Embarco/Desembarco como .docx (mismas columnas que la
// versión imprimible), para poder editarla en Word antes de presentarla.
const COLS_PLANILLA = [
  "NOMBRE Y APELLIDO","DNI","HABILITACION","FOJA","CENSO","R.MED",
  "FECHA EMBARCO","FECHA DESEMBARCO","FOJA MOV.\nLIBRO ROL","EMPLEO\nA BORDO",
  "SING.","N° LE","VENC.\nCEDULA",
];
function celdaPlanilla(texto, bold=false) {
  return new TableCell({
    children: String(texto ?? "").split("\n").map(linea => new Paragraph({ children: [new TextRun({ text: linea, bold, size: 16 })] })),
    margins: { top: 40, bottom: 40, left: 60, right: 60 },
  });
}
async function descargarPlanillaWord(proyecto, enc, filas) {
  const tabla = new Table({
    width: { size: 100, type: WidthType.PERCENTAGE },
    rows: [
      new TableRow({ tableHeader: true, children: COLS_PLANILLA.map(c => celdaPlanilla(c, true)) }),
      ...filas.map(f => new TableRow({ children: [
        celdaPlanilla(f.emp.apellido_nombre),
        celdaPlanilla(f.emp.dni),
        celdaPlanilla(f.habilitacion),
        celdaPlanilla(""),
        celdaPlanilla(f.censo),
        celdaPlanilla(f.rmed),
        celdaPlanilla(f.tipo==="embarca" ? fmtDate(f.asign.fecha_desde) : ""),
        celdaPlanilla(f.tipo==="desembarca" ? fmtDate(f.asign.fecha_hasta) : ""),
        celdaPlanilla(""),
        celdaPlanilla(catsLabel(f.emp)),
        celdaPlanilla(f.singladura!=null ? String(f.singladura) : ""),
        celdaPlanilla(f.emp.libreta),
        celdaPlanilla(f.vencCedula),
      ] })),
    ],
  });
  ...
  const doc = new Document({
    sections: [{
      ...
      children: [
        new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: "PLANILLA EMBARCO/DESEMBARCO", bold: true, size: 24, underline: {} })] }),
        new Paragraph({ text: "" }),
        new Paragraph({ children: [new TextRun({ text: "AGENCIA MARÍTIMA: ", bold: true }), new TextRun(enc.agencia || "")] }),
        new Paragraph({ children: [
          new TextRun({ text: "BUQUE: ", bold: true }), new TextRun(proyecto.buque + "    "),
          new TextRun({ text: "MAT: ", bold: true }), new TextRun((enc.mat||"") + "    "),
          new TextRun({ text: "BANDERA: ", bold: true }), new TextRun(enc.bandera || ""),
        ] }),
        new Paragraph({ children: [new TextRun({ text: "RESPONSABLE PNA: ", bold: true }), new TextRun(enc.responsable || "")] }),
        new Paragraph({ text: "" }),
        tabla,
      ],
    }],
  ...
  a.href = url;
  a.download = `Planilla Embarco-Desembarco - ${proyecto.buque} - ${proyecto.nombre}.docx`;
...
function ModalPlanillaEmbarque({ proyecto, movimientos, duplicados, tiposDoc, documentos, onClose }) {
  ...
  }).sort((a,b) => (a.tipo===b.tipo ? 0 : a.tipo==="embarca" ? -1 : 1) || a.emp.apellido_nombre.localeCompare(b.emp.apellido_nombre));
  ...
  return (
    <div className="overlay">
      <div className="modal" style={{maxWidth:1150}}>
        ...
        <div className="mftr no-print">
          <button className="btn btn-ghost" onClick={onClose}>Cerrar</button>
          <button className="btn btn-ghost" onClick={()=>descargarPlanillaWord(proyecto, enc, filas)} disabled={filas.length===0}>Descargar Word</button>
